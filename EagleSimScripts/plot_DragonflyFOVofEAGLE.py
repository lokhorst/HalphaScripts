...
if __name__ == "__main__":
    ...
    if mockobs and filament:
        ...
        plotit(10**4 * 3600,ax2, map, 100)
        #ax2.locator_params(axis='y', nticks=3)
        ax2.set_xticklabels([])
        ax2.set_xlabel('')
        #ax2.set_xticks([])
        label = '500" resolution'
        plotit(10**4 * 3600,ax3, map, 500)
        ax3.set_xticklabels([])
        ax3.set_xlabel('')
        #ax3.set_xticks([])
        label = '1000" resolution'
        plotit(10**4 * 3600,ax4, map, 1000)
        ax4.set_xticklabels([])
        ax4.set_xlabel('')
        #ax4.set_xticks([])
        #plt.tight_layout()
        ...
        # once the data are loaded at the different distances, plot them
        ...
        for distance,axistop,axisbot in zip(['50Mpc','100Mpc','200Mpc','500Mpc'],[ax1,ax2,ax3,ax4],[ax11,ax22,ax33,ax44]):
            ...
            #get_halpha_SB.
            makemap(data_FOV,size,axistop,xystarts = xystarts)
