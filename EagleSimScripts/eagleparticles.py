...
class GasVelocities_ReadEagle:
    ...
    def read_galaxy(self, itype, gn, sgn, centre, velocity, load_region_length, fileloc):
        ...
        for att in ['GroupNumber', 'SubGroupNumber', 'StarFormationRate', 'Temperature', 'Density', 'Coordinates',
                    'Velocity','SmoothedElementAbundance/Oxygen','SmoothedElementAbundance/Hydrogen']:
    ...
    def plot(self, gn, sgn):
        ...
        ### would it be straightforward to add the emission calculation here, and then sort by H-alpha emission luminosity as well?  
        ### not really -- need to use a c function to interpolate the emissivity table -- doable but maybe save it for later if this stuff isn't clear (if split by temp, density etc)
